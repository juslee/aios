...
use crate::ipc::CHANNEL_TABLE;
use crate::syscall::IpcError;
use crate::task::ThreadId;
use shared::{ChannelId, MAX_CHANNELS};
...
///
/// `denied` holds a queued message while the EPERM cases run, so a check
/// that ran after the non-blocking scan would report it ready instead of
/// returning EPERM. Every select uses a short timeout, so a regression that
/// lets one block fails its check with ETIMEDOUT, or at worst stalls this
/// thread before the success line prints. The EPERM cases each log one
/// expected `denied ChannelAccess` warning from the capability check.
pub(super) fn select_cap_test(my_tid: ThreadId) {
    use crate::ipc::select::{ipc_select, SELECT_WAITERS};
    use shared::{Capability, RawMessage, SelectEntry, SelectKind};
    ...
    // goes straight onto the ring. Selecting on it must still return EPERM.
    let queued = {
        let mut table = CHANNEL_TABLE.lock();
        crate::ipc::channel_mut(&mut table, denied).is_ok_and(|ch| {
    ...
    // A set with only an inaccessible (and ready) channel.
    let denied_only = ipc_select(&[chan(denied)], SELECT_TIMEOUT_TICKS);
    if denied_only != eperm {
        passed = false;
        crate::kwarn!(Ipc, "Select-cap: denied -> {:?}", denied_only);
    }

    // A mixed set. The accessible channel comes first, so a check made while
    // registering would already have claimed its waiting_receiver slot.
    let mixed = ipc_select(&[chan(owned_a), chan(denied)], SELECT_TIMEOUT_TICKS);
    let on_channel = {
        let mut table = CHANNEL_TABLE.lock();
        crate::ipc::channel_mut(&mut table, owned_a).is_ok_and(|ch| ch.waiting_receiver.is_some())
    };
    ...
    if send_result != 0 || owned != Ok((1, 0)) || !drained {
        passed = false;
        crate::kwarn!(Ipc, "Select-cap: owned -> {} {:?}", send_result, owned);
    }
    ...
    // Drop the queued message so `denied` is left empty.
    if let Ok(ch) = crate::ipc::channel_mut(&mut CHANNEL_TABLE.lock(), denied) {
        ch.ring.pop();
    }
