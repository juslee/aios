...
impl KeyCode {
    ...
    /// Translate this `KeyCode` back to its evdev keycode.
    ///
    /// Inverse of `from_evdev`: every named variant returns its canonical
    /// evdev code, and `KeyCode::Unknown(c)` returns `c`. Used for IPC
    /// transport where the wire format is the evdev code (a stable u16).
    pub fn to_evdev(self) -> u16 {
        match self {
            KeyCode::A => KEY_A,
            KeyCode::B => KEY_B,
            KeyCode::C => KEY_C,
