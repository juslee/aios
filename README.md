...
Phase 0 (toolchain and bare-metal boot), Phase 1 (boot and first pixels), and Phase 2 (memory management) are complete. Phase 3 is in progress with Milestones 10 and 11 complete. The kernel boots via edk2 firmware on QEMU virt with 4 SMP cores online, a framebuffer test pattern rendered, full TTBR1 kernel page tables with W^X enforcement, a slab-backed kernel heap (`kalloc`/`kfree`), and per-agent address spaces with TTBR0 switching. Milestone 10 added structured per-core kernel logging (`klog!` macros), thread and process control structures, SVC-based syscall dispatch with all 31 syscall numbers defined, and a 1 kHz timer tick driving preemption flags and log drain. Milestone 11 added a 4-class per-CPU scheduler with timer-driven preemption, synchronous IPC channels with call/reply semantics, a direct-switch fast path bypassing the scheduler for IPC, priority inheritance across IPC boundaries, and a capability system enforcing per-process access control on all IPC and channel operations.
