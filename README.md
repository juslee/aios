...
Phase 0 (toolchain and bare-metal boot) and Phase 1 (boot and first pixels) are complete. Phase 2 (memory management) is in progress with M7 (physical memory manager) and M8 (virtual memory and KASLR) complete. The kernel boots via edk2 firmware on QEMU virt with 4 SMP cores online, a framebuffer test pattern rendered, and full TTBR1 kernel page tables with W^X enforcement active.
