...
The Service Manager (boot Phase 4-6) uses the async executor to start services concurrently. Each service starts as an async task that:
...
Phase 28 — Power Management and Thermal:
...
Each phase builds on the previous. The Phase 1 scheduler is trivial — just enough to boot. By Phase 22, the scheduler handles all four classes, fair-share, context awareness, and multi-core balancing. Phase 28 adds power management, the last piece needed for a production-quality scheduler on real hardware.
...
**Target.** Phase 22+ (requires stable observability metrics as optimization objective — see observability.md §10).
...
**Target.** Phase 28+ (after power management stabilizes and compositor deadlines are established).
