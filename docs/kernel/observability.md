...
graph TD
    subgraph Consumers
        Inspector["Inspector<br/>(Phase 18)"]
        AIRS["AIRS<br/>(Phase 10)"]
        UART["UART Console<br/>(Phase 3)"]
    end

    subgraph Access["Access Layer"]
        AuditRead["AuditRead(Scope::All)<br/>AuditRead(Scope::Operational) — Phase 18"]
...
2. **KernelInfoPage extension (Phase 18)** — A snapshot of recent trace events can be exported through the shared memory page for Inspector visualization.
...
These two streams share the same **access interface** (`AuditRead` syscall, capability-gated) starting in Phase 18, but they are stored separately. Merging operational logs into the Merkle chain would bloat it by 100× and add no security value — kernel log entries are not agent actions.
...
### 6.4 AuditRead Extension (Phase 18)
...
| **Phase 18** (Security) | `AuditScope::Operational`, log-to-provenance bridge | Operational data accessible through `AuditRead` capability system. |
...
**Target.** Phase 22+ (requires stable trace infrastructure and sufficient historical data for pattern learning).
...
**Target.** Phase 22+ (requires stable VirtIO-blk driver and block engine from Phase 4).
...
**Target.** Phase 22+ (requires stable metrics infrastructure, hardware counters, and sufficient workload diversity for training).
