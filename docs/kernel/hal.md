...
USB is the primary input device path on Raspberry Pi hardware and provides expansion connectivity on all platforms. The full USB architecture — including xHCI/DWC2 controller internals, device enumeration, class drivers, hotplug, security, and AI-native features — is documented in the dedicated USB subsystem document.

**See:** [USB Subsystem Architecture](../platform/usb.md) — full documentation including:

- [Controller Architecture](../platform/usb/controller.md) — `UsbHostController` trait, xHCI ring buffers, DWC2 channels, DMA management, interrupt handling (§2)
- [Device Enumeration and Class Drivers](../platform/usb/device-classes.md) — enumeration state machine, HID, mass storage, audio, video, network, serial drivers (§3-§5)
- [Hotplug and Power Management](../platform/usb/hotplug.md) — hub enumeration, hotplug state machine, USB-C Power Delivery (§6-§8)
- [Security and AI-Native](../platform/usb/security.md) — threat model, IOMMU isolation, device allowlisting, anomaly detection (§9-§11)

The `PlatformUsb` extension trait remains in this document (§12) as it follows the HAL extension trait pattern.
