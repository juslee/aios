...
**Related:** [memory-physical.md](./memory-physical.md) — Physical memory, [memory-ai.md](./memory-ai.md) — Model memory, [ipc.md](./ipc.md) — IPC channels
...
impl AddressSpace {
    ...
    /// encodings (see [§10.5](./memory-reclamation.md) for PteState decoding).
...
pub struct AgentProcess {
    ...
    /// Agent priority from manifest ([§8](./memory-reclamation.md)).
    /// Used by OOM scorer ([§8](./memory-reclamation.md)) and thrash detector ([§10.6](./memory-reclamation.md)) for victim selection.
...
The agent is never silently killed except in OOM conditions ([§8](./memory-reclamation.md)). Pausing preserves the agent's state so it can resume if the user increases the limit.
...
/// Called from the page fault dispatcher ([§10.5](./memory-reclamation.md)) with the faulting address,
