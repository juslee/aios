...
**Related:** [memory-virtual.md](./memory-virtual.md) — Virtual memory, [memory-ai.md](./memory-ai.md) — Model memory, [memory-reclamation.md](./memory-reclamation.md) — Reclamation and swap
...
//
//   PhysicalAddress    — newtype wrapper around usize (defined in [§3.2](./memory-virtual.md))
//   PhysicalFrame      — single physical page frame, identified by PFN (§2.2)
//   VirtualAddress     — newtype wrapper around usize ([§3.2](./memory-virtual.md))
//   PageTableEntry     — 64-bit aarch64 PTE with W^X helpers ([§3.2](./memory-virtual.md))
//   PageTable          — 512-entry page table ([§3.2](./memory-virtual.md))
//   AddressSpace       — per-process virtual address space ([§3.2](./memory-virtual.md))
...
impl FrameAllocator {
    ...
    /// allocated and excluded from pressure calculations ([§8](./memory-reclamation.md)).
    pub fn pressure(&self) -> MemoryPressure {
...
| Model | Yes (boundary with User) | AIRS grows model pool when loading models, shrinks when evicting. Constrained by `security_floor` ([§12.2](./memory-reclamation.md)). |
