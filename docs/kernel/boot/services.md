...
**Related:** [kernel.md](./kernel.md) — Kernel early boot, [performance.md](./performance.md) — Boot timing, [recovery.md](./recovery.md) — Recovery mode, [lifecycle.md](./lifecycle.md) — Shutdown
...
t=0ms:   Start device_registry (no dependencies within phase)
         (posix_compat and audio_subsystem are on-demand — not started here;
          see intelligence.md §17 for activation modes)
...
3. Wi-Fi Configuration (if network not already connected)
   ...
   - If connected: time sync via NTP (see performance.md §6.5)
