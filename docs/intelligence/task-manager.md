...
The Task Manager starts during **Phase 6 (Experience)** of the boot sequence. It depends on services from Phase 3 (AI Services) and Phase 4 (User Services) being available.
...
Phase 4: User Services (critical path continues)
  ...
  │       └──→ agent_runtime
  │
Phase 6: Experience (starts when Phase 4 completes)
...
AIRS may not be ready when the Task Manager starts (Phase 6 does not wait for Phase 3). The Task Manager handles this gracefully:
...
Phase 15: Task Decomposition and Execution
...
Phase 17: Agent Orchestration and Concurrency
...
Phase 22: Optimization and Intelligence
...
Phase 22-23: Security and Observability (§14-§15)
...
Phase 26-28: AI-Native Intelligence (§18-§19)
...
Phase 30+: Multi-Device and Power-Aware (§16-§17)
...
- Task Manager requires Agent Runtime (Phase 14) — cannot spawn task agents without it.
- Task Manager requires Space Storage (Phase 4) — task state persisted in `system/tasks/`.
- AIRS integration requires AIRS inference engine (Phase 10) — intent decomposition needs inference.
- Context-aware scheduling requires Context Engine (Phase 12) — priority hints from context state.
- Workspace display requires Compositor (Phase 7) — task progress shown in the Workspace UI.
