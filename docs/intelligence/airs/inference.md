...
**Kit overview:** [AIRS Kit](../../kits/intelligence/airs.md) — AI Runtime Service Kit; [Compute Kit](../../kits/kernel/compute.md) — Tier 3 (Inference Pipeline) defines the app-facing `InferencePipeline` trait that this engine implements
...
The inference engine runs behind Compute Kit Tier 3's `InferencePipeline` trait ([Compute Kit](../../kits/kernel/compute.md)), which defines the AIOS-native abstraction for inference workloads: NPU→GPU→CPU fallback, session management, and streaming token delivery. The default bridge above this trait is **candle** — a pure Rust ML inference runtime by Hugging Face — chosen for memory safety, build simplicity, and alignment with the "Custom Core, Open-Source Bridges" principle. GGML (a C inference library) can serve as an alternative bridge via FFI if performance on specific hardware requires hand-tuned assembly kernels (see §3.9 for comparison and the `InferenceRuntime` trait abstraction that makes the backend swappable).
...
/// Panics are caught with catch_unwind; no signal handler needed.
pub struct CandleBackend {
    /// candle device (CPU with NEON, or Metal/CUDA if available).
    device: candle_core::Device,

    /// Memory pool backing model weights. Allocated from Pool::Model
    /// via the kernel's frame allocator. Pinned for the model lifetime.
    ...
    /// Watchdog handle for hang detection (§3.7.2).
    /// If a forward pass exceeds the expected duration, triggers recovery.
...
Each inference call (prompt encoding + token generation) builds a compute graph — a DAG of tensor operations that the runtime executes. With candle, graph construction and execution are unified in Rust; with GGML, the graph is built in Rust and executed by the C library:
...
1. **Build** (Rust): Construct the graph from the model architecture. Each transformer layer adds attention, feed-forward, and normalization nodes. The graph is rebuilt for each forward pass (compute graphs are ephemeral).

2. **Plan** (Runtime): Allocate scratch memory for intermediate tensors. The runtime uses a memory pool (not `malloc`) — the pool is pre-allocated from `Pool::Kernel` at graph build time. Schedule operations across available threads.

3. **Execute** (Runtime + NEON): Run the graph. Matrix multiplications use NEON SIMD kernels (§3.1.4). KV cache is read for past context and written with new key-value pairs. The thermal yield point (§3.1.5) is checked between layers.
...
AIOS targets aarch64 exclusively, so all inference compute kernels use ARM NEON SIMD instructions. NEON provides 128-bit vector registers (32 registers, `v0`–`v31`), enabling 4 FP32 or 16 INT8 operations per instruction. candle's NEON backend (`candle-core`) provides these kernels; GGML's hand-tuned assembly offers an alternative with slightly higher throughput on some operations.

**Key NEON kernels for quantized inference:**
...
Scratch buffers       Runtime          Pool::Kernel         Graph build→free   Never shared
```
...
- **Scratch buffers** are temporary memory used by the runtime during graph execution. Allocated from `Pool::Kernel` at graph build time, freed immediately after the forward pass completes. Never shared between sessions.

**Why candle as the default bridge:**

- **Pure Rust** — no FFI boundary, no unsafe code in the inference path, compiler-enforced memory safety
- **GGUF model support** — compatible with the same quantized model files via the `gguf` crate
- Optimized quantization (Q4_K_M, Q5_K_M, Q6_K) — 7B models fit in 4-6 GB RAM
- NEON SIMD backend for aarch64 (the only architecture AIOS targets)
...
flowchart LR
    subgraph Forward["Forward Pass"]
        FP[candle compute graph] --> LO[Raw logits<br>vocab_size floats]
    end
...
pub trait TokenCallback: Send {
    ...
    /// Called on error (runtime panic, thermal shutdown, budget exceeded, timeout).
...
1. Stops token generation immediately (after the current compute graph completes).
...
pub enum PauseReason {
    ...
    ThermalThrottle,
    /// Inference runtime needs to swap model weights (rare).
    ModelSwap,
}
```
...
The inference engine must handle errors from multiple sources: runtime crashes (candle panics or GGML segfaults), memory exhaustion, thermal shutdown, budget violations, and device failures. The error handling architecture ensures AIRS continues operating even when individual inference sessions fail catastrophically.
...
pub enum InferenceError {
    // --- Runtime errors (from candle or GGML backend) ---
    /// Runtime returned an error from graph computation.
    RuntimeComputeError { message: String },
    /// Runtime panicked (candle: catch_unwind) or crashed (GGML: signal handler).
    RuntimePanic { details: String },
    /// Runtime hung — watchdog timeout exceeded.
    RuntimeTimeout { elapsed: Duration },
...
#### 3.7.2 Runtime Crash Containment

The crash containment strategy depends on which backend is active:

**candle (default — pure Rust):** A candle failure is a Rust panic, caught with `catch_unwind` and converted to `InferenceError::RuntimePanic`. No signal handlers, no `longjmp`, no C-level crash recovery. This eliminates ~200 lines of unsafe code compared to the GGML path.

**GGML (alternative — C FFI):** GGML is C code — it can segfault, buffer overflow, or enter infinite loops. If the GGML backend is active, a `CrashContainment` system installs signal handlers (SIGSEGV/SIGBUS/SIGFPE/SIGABRT), catches crashes via `longjmp` back to the safe Rust boundary, and converts them to `InferenceError::RuntimePanic`.
...
**Recovery sequence (both backends):**

1. **Error caught**: candle panic via `catch_unwind`, or GGML signal via handler.
2. **Convert to error**: Converted to `InferenceError::RuntimePanic`.
3. **Session cleanup**: The failed session's KV cache is freed. The session transitions to `Failed` state.
4. **Runtime validation**: The runtime runs a self-check (small test inference). If it passes, the runtime is reused. If it fails, the runtime is torn down and re-initialized.
...
**Watchdog recovery** follows the same sequence but with step 1 replaced by a timer expiration. For candle, the stuck thread is cancelled cooperatively. For GGML, `pthread_cancel` is used as a last resort.
...
### 3.9 Inference Runtime Bridges

The inference engine runs behind Compute Kit Tier 3's `InferencePipeline` trait. The runtime that executes model forward passes is a **bridge above the Kit** — swappable without changing AIRS, the session manager, or the streaming pipeline. See [ADR: Compute Kit](../../knowledge/decisions/2026-03-22-jl-compute-kit.md).

#### 3.9.1 candle (Default Bridge)

candle (by Hugging Face) is the default inference bridge — a pure Rust ML inference runtime that aligns with the "Custom Core, Open-Source Bridges" principle ([discussions/2026-03-16-jl-platform-vision-custom-core.md](../../knowledge/discussions/2026-03-16-jl-platform-vision-custom-core.md) §6).

**Why candle is the default:**

- **Pure Rust** — no FFI boundary, no unsafe code, compiler-enforced memory safety
- **Simplified crash containment** — panics caught with `catch_unwind`, eliminating ~200 lines of signal handler code
- **Pure Cargo build** — no cmake, no C compiler, no system dependencies
- **GGUF compatible** — reads the same quantized model files via the `gguf` crate
- NEON SIMD backend for aarch64; Metal and CUDA backends for GPU acceleration
- Hugging Face ecosystem with rapid model format adoption

**Known trade-off:** candle's NEON SIMD kernels are not yet as optimized as GGML's hand-tuned assembly. On ARM hardware (AIOS's primary target), GGML may generate tokens 5-15% faster. This gap is narrowing as candle's NEON backend improves.

#### 3.9.2 GGML (Alternative Bridge)

GGML can be wired as an alternative backend via FFI if hand-tuned NEON performance is required:

```text
Aspect                  candle (default)             GGML (alternative)
─────────────────────   ────────────────────────────  ─────────────────────────
Language                Pure Rust                    C
FFI required            No                           Yes (unsafe boundary)
GGUF model support      Via gguf crate (compatible)  Native
ARM NEON SIMD           candle-core NEON backend     Hand-optimized assembly
Metal GPU               Via candle-metal              Via ggml-metal
CUDA GPU                Via candle-cuda               Via ggml-cuda
Crash containment       Rust panics (catch_unwind)   Signal handler (§3.7.2)
Memory safety           Compiler-enforced            C-level (manual)
Build system            Pure Cargo (simple)          C + cmake (complex)
Performance (ARM)       ~85-95% of GGML on NEON     Reference implementation
```

The GGML backend requires an `ggml_sys` FFI module that confines all unsafe code to a single boundary, plus a `CrashContainment` system with signal handlers and `longjmp` recovery (§3.7.2).

#### 3.9.3 InferenceRuntime Trait

All AIRS code interacts with the `InferenceRuntime` trait, not with candle or GGML APIs directly. This makes the backend swappable at initialization time:

```rust
/// Abstraction over the inference runtime (candle, GGML, or future backends).
/// All AIRS code interacts with this trait, not with runtime-specific APIs.
/// This trait is the bridge interface — Compute Kit Tier 3's InferencePipeline
/// trait is the app-facing API above.
...
pub struct CandleBackend {
    // Default bridge — no crash containment needed, Rust panics are sufficient
    device: candle_core::Device,
    models: HashMap<ModelHandle, candle_transformers::Model>,
}

pub struct GgmlBackend {
    // Alternative bridge — requires FFI safety layer + crash containment
    runtime: GgmlRuntime,
...
**Backend selection:** At AIRS boot, the inference engine instantiates whichever backend is configured (candle by default). The choice can be overridden via system configuration for benchmarking or hardware-specific optimization. Both backends read the same GGUF model files.

#### 3.9.4 Other Runtimes Considered
...
| Compute Kit ADR | [ADR: Compute Kit](../../knowledge/decisions/2026-03-22-jl-compute-kit.md) | Reference — 3-tier model decision |
