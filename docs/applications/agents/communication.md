...
**Capability enforcement at every hop.** The kernel checks `ChannelAccess(channel_id)` on every `ipc_call`, `ipc_send`, and `ipc_recv`, and on every channel in an `ipc_select` wait set. An agent cannot send to or wait on a channel it does not hold a capability for. Capability delegation enables controlled communication graphs — Agent A can grant Agent B access to a specific channel without granting access to all of Agent A's channels. See [model/capabilities.md](../../security/model/capabilities.md) §3.3 for attenuation rules.
