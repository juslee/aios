...
The screen reader does not read pixels. It reads the accessibility tree maintained by the compositor (see [compositor.md](../../platform/compositor.md) §9.1) and populated by the UI toolkit (see [interface-kit.md](../../applications/interface-kit.md) §12.1). Every widget has a semantic role, a label, and a state. The screen reader converts these into natural speech:
