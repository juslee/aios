...
| **Phase 27 target** | QEMU (complete), Pi via rpi4-uefi | Pi 4, Pi 5 | Apple M1/M2/M3/M4 |
...
AIOS's position in this chain: trust the platform firmware to do its job. AIOS does not attempt to verify the firmware, re-verify m1n1, or duplicate the platform's trust chain. AIOS verifies `BootInfo.magic` at entry and, in future phases, will verify the kernel image hash via the capability system once the security model (Phase 28) is implemented.
