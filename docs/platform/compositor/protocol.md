...
pub enum CompositorEvent {
    ...
    /// The release fence (if present) signals when the compositor has finished reading.
    BufferReleased {
        buffer: SharedBufferId,
...
**Release fence:** Generated by the compositor and delivered via `BufferReleased`. The `release_fence` field contains the fence ID; the agent must not write to the buffer until this fence signals. In practice, the compositor signals the release fence at the same time it sends `BufferReleased`, so agents that only listen for the event (ignoring the fence object) behave correctly. For CPU-rendered surfaces, `release_fence` is `None` — the `BufferReleased` event itself is sufficient.
...
**When hints use defaults:** A surface created with default `SurfaceHints` (all fields set to their default values) is treated as `Browser` content type with `Flexible` layout preference, `Active` interaction state, `Normal` layer, `None` urgency, and `Primary` semantic role. This combination produces reasonable behavior for any content — the compositor tiles it like a generic application window with no special treatment.
