...
The USB subsystem is implemented in **Phase 17** (5 weeks) across three milestones. Prerequisites: Phase 16 (Camera) completes the subsystem framework pattern; Phase 3 (IPC/Scheduler) provides the threading model.
...
Phase 12 (HAL) ────────── PlatformUsb trait, IOMMU abstraction
Phase 16 (Camera) ─────── Subsystem Framework pattern established
     │
Phase 17 (USB) ────────── This document
     │
Phase 18 (WiFi/BT) ───── Wireless subsystems (USB dongle support)
Phase 19 (Power) ──────── Full USB-C PD integration
```

### Cross-Phase USB Features

Some USB features are implemented in phases other than Phase 17:

- **USB Audio Class driver**: Phase 18 (audio/drivers.md §5.6)
- **USB HID Braille displays**: Phase 17+ (accessibility.md §4)
- **USB-C Power Delivery negotiation**: Phase 19 (power-management.md)
- **USB4/Thunderbolt tunneling**: Phase 22+ (§14 Future Directions)
- **AI-native USB features**: Phase 29+ (security.md §10)
