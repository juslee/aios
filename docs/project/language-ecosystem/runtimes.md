...
|---|---|---|
| `aios-sdk` crate | Built with AIOS | Phase 14 |
| `#[agent]` proc macro | Generates entry point + manifest parsing | Phase 14 |
| Rust compiler (cross) | rustc on host, target aarch64-unknown-none | Phase 0+ |
| Rust compiler (native) | rustc running ON AIOS | Phase 23+ |
...
Until LLVM runs natively on AIOS (Phase 23f), `rustc` can't run natively either. The practical
path: cross-compile `rustc` on the host and ship it as a pre-built AIOS binary, then later
achieve true self-hosting.

### Development Workflow (Phase 17+)
...
# On AIOS (Phase 23f+) — once rustc is available natively
...
|---|---|---|---|
| RustPython interpreter | github.com/RustPython/RustPython | MIT | Phase 17 |
| RustPython embedding API | github.com/RustPython/RustPython | MIT | Phase 17 |
| `aios-sdk` pip package | Built with AIOS | BSD-2-Clause | Phase 17 |
| Agent-local `site-packages/` | Declared in manifest, installed at install time | — | Phase 17 |
...
| Sandbox integration | Compiles into agent binary | Requires POSIX layer (Phase 23) |
| Available at | Phase 17 | Phase 23 (needs POSIX) |
...
After Phase 23, CPython becomes available through the POSIX layer for workloads that need C
extension compatibility (numpy, etc.).

### Self-Hosting: When Can You Write Python ON AIOS?

**Phase 17.** RustPython ships with the OS. You can write and run Python agents directly on AIOS
from Phase 17 onward. No cross-compilation needed — Python is interpreted.
...
arriving 3 phases before C/C++ (Phase 23) and ~4 phases before Rust (Phase 24+).
...
| Component | Source | License | Phase |
|---|---|---|---|
| QuickJS-ng engine | github.com/quickjs-ng/quickjs | MIT | Phase 17 |
| napi-like bridge | Custom, built with AIOS | BSD-2-Clause | Phase 17 |
| `@aios/sdk` npm package | Built with AIOS | BSD-2-Clause | Phase 17 |
| TypeScript transpiler | Bundled (runs at install time) | Apache-2.0 | Phase 17 |
...
| Available at | Phase 17 | Phase 17 (alternative) | Phase 23 (via Node.js on POSIX) |

**QuickJS-ng** is chosen over Boa for Phase 17 because of its superior runtime performance
...
For compute-heavy JavaScript (browser workloads), SpiderMonkey arrives in Phase 31 via Servo.

### Self-Hosting: When Can You Write TypeScript ON AIOS?

**Phase 17.** QuickJS-ng ships with the OS. TypeScript transpilation happens at install time.
You can write and run TypeScript agents directly on AIOS from Phase 17 onward.
...
| Component | Source | License | Phase |
|---|---|---|---|
| wasmtime runtime | github.com/bytecodealliance/wasmtime | Apache-2.0/MIT | Phase 17 |
| WASI-to-AIOS bridge | Custom — maps WASI imports to AIOS IPC | BSD-2-Clause | Phase 17 |
| AOT compiler | wasmtime's Cranelift (compiles .wasm → native at install) | Apache-2.0 | Phase 17 |

### WASI Standards Timeline

| Standard | Status | Key Feature | AIOS Relevance |
|---|---|---|---|
| WASI 0.2.0 | Stable (January 2024) | Component Model foundation | Phase 17 baseline |
...
**Agent WASM (Phase 17):** WASM modules run in wasmtime inside the agent sandbox. Double-sandboxed:
...
**Browser WASM (Phase 31):** WASM runs inside SpiderMonkey (via Servo) within Tab Agents.
...
- **Rust → WASM**: Needs `rustc` with `wasm32-wasi` target (Phase 24+)
- **C → WASM**: Needs clang with `wasm32-wasi` target (Phase 23f)
- **AssemblyScript → WASM**: Needs Node.js or QuickJS-ng-compatible tooling (Phase 17+)
