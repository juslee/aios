...
|---|---|---|
| AI-assisted (current) | ~50–70 weeks | Solo developer with Claude Code, architecture-first workflow |
...
flowchart LR
    ...
    *Capability profiles, AIRS cap intelligence*`"]
    ...
    style T1a fill:#FFA500,color:white
```
...
| 1.5 | Week 4 | Kit Foundation | Storage complete, Kit crate structure established, 4 Kit APIs extracted |
| 2 | Week 11 | Graphical desktop with shell | Window compositor, terminal emulator, basic networking (curl works) |
| 3 | Week 23 | AI-enhanced OS | Conversation bar, semantic search, agents running with capability gates, Flow service, identity system |
...
flowchart TD
    ...
    P10 --> P11["Phase 11: AIRS Inference Engine"]
    ...
    P12 --> P20
    P10 --> P21["Phase 21: Security Architecture"]
    P4 --> P21
    P20 --> P21
    ...
    style P5 fill:#1E90FF,color:white
```
...
- **Intelligence chain:** 14 → 15 → 16 → 17 (context → preferences → flow → attention)
...
**Decision:** Is the full intelligence stack operational? Can agents, search, conversation, and identity work together?

**Criteria:**

- Semantic search returns relevant results across spaces (Space Indexer + Search Kit)
- Conversation bar accepts natural language queries and invokes tools (Conversation Manager)
- Agents can sign actions with identity credentials (Identity Kit)
- Intent verification pipeline validates agent behavior (prerequisite for Gate 4 → Phase 20)
- All Kit APIs extracted for intelligence-layer Kits

**If NO:** Identify which intelligence subsystem is blocking and allocate additional time before Tier 4. Intent Verification (Phase 20) depends on a functional intelligence stack.

### Gate 5: Hardware-Ready (after Phase 33)

**Decision:** Does AIOS run on real hardware with full connectivity?

**Criteria:**

- Boots and runs stable on Raspberry Pi 4/5
- WiFi and Bluetooth functional (WPA2/WPA3, audio + HID)
- Thermal throttling prevents overheating under sustained load
- USB hotplug works (keyboard, mouse, storage)
- Power management: sleep and wake functional

**If NO:** Identify hardware-specific issues. Budget additional time for driver debugging before Tier 7.
...
- Developer A: kernel (Phases 0–3), then Kit Foundation (5), then compute abstraction (23–24), then performance (25), then security (21–22)
- Developer B: storage + GPU (Phases 4, 6), then compositor/input/networking (7–9), then UI toolkit (34), then browser (35) — ordered to respect dependency chain (Phase 26 → 34 → 35)
- Developer C: AI (Phases 10–19), then networking (28), then agent ecosystem (26)
- Note: Remaining phases (20, 27, 29–33, 36–44) are assigned based on availability as earlier phases complete.
...
| 5 | Kit Foundation | Memory Kit, IPC Kit, Capability Kit, Storage Kit (API extraction) | kits/README.md | `05-kit-foundation.md` | Planned | 3w | — |
| 6 | GPU & Display | Compute Kit (Tier 1) | gpu.md (5) | `06-gpu-and-display.md` | Planned | 6w | — |
| 7 | Window Compositor & Shell | — (Compositor is a service) | compositor.md (6) | `07-window-compositor-and-shell.md` | Planned | 7w | — |
...
| 12 | AIRS Intelligence Services | AIRS Kit (behavioral monitor, tool mgr, lifecycle) | airs/intelligence-services.md, lifecycle-and-data.md, security.md, ai-native.md | `12-airs-intelligence-services.md` | Planned | 4w | — |
...
| 22 | Security Testing & Verification | — (cross-cutting) | fuzzing.md (4) | `22-security-testing-and-verification.md` | Planned | 4w | — |
...
| 25 | Performance & Optimization | — (cross-cutting) | — | `25-performance-and-optimization.md` | Planned | 4w | — |
| 26 | Developer Experience & SDK | Cookbook | — | `26-developer-experience-and-sdk.md` | Planned | 5w | — |
| 27 | POSIX Compatibility & BSD Userland | — (cross-cutting) | posix.md | `27-posix-compatibility-and-bsd-userland.md` | Planned | 5w | — |
...
| 38 | Accessibility & Internationalization | — (cross-cutting) | accessibility.md | `38-accessibility-and-internationalization.md` | Planned | 5w | — |
...
|---|---|---|---|
| Subsystem Framework | Phases 6–9 (established incrementally) | Every subsystem phase (28–33) | `subsystem-framework.md` |
| Device Model | Phases 0–6 (core), 28–37 (drivers) | Every driver phase | `device-model.md` (7 sub-docs) |
| Preference System | Phase 15 | 7 (display config), 21 (security prefs), 9 (network prefs), 34 (UI prefs), 38 (a11y prefs) | `preferences.md` (8 sub-docs) |
| Identity | Phase 19 (primary) | 3 (capability foundation), 21 (security integration), 42 (multi-device maturity) | `identity.md` (8 sub-docs) |
| Experience Layer | Phase 7 (foundation) | 15, 17, 18, 34, 38 | `experience.md` |
| Kernel Compute Abstraction | Phase 23 | 33 (thermal coupling), 24 (accelerators), 46 (AIRS intelligence) | `compute.md` (6 sub-docs) |
| Language Ecosystem | All phases (Rust) | 10 (agent runtimes), 26 (WASM/SDK), 46 (AIRS Runtime Advisor) | `language-ecosystem.md` (4 sub-docs) |
| Search Kit | Phase 13 (primary) | 14 (context engine integration), 18 (conversation search) | `space-indexer.md` (8 sub-docs) |
| Conversation Kit | Phase 18 (primary) | 13 (search), 14 (context), 17 (attention) | `conversation-manager.md` (6 sub-docs) |
| Intent Kit | Phase 20 (primary) | 21 (security integration), 12 (behavioral monitor) | `intent-verifier.md` (6 sub-docs) |
| Translation Kit | Phase 16 (bundled with Flow) | 18 (conversation format) | `flow.md` (6 sub-docs) |
| Notification Kit | Phase 17 (bundled with Attention) | 18 (conversation alerts) | `attention.md` |
| Security Kit (app layer) | Phase 21 (bundled with Security Architecture) | 20 (intent verification) | `model.md` (5 sub-docs) |

These subsystems do not have their own dedicated phases — their work is distributed across the phases listed above. Each primary phase is responsible for establishing the subsystem's core abstractions; subsequent phases extend them. Kits that are bundled with related subsystems (Translation, Notification, Security Kit) share a phase with their parent subsystem due to tight coupling.
...
### Tier 1.5 + Tier 2 Complete (Week 11)

- [ ] Kit crate structure established with trait conventions for Memory/IPC/Capability/Storage Kits
...
### Tier 3 Complete (Week 23)

- [ ] Capability-gated agents with SDK and Scriptable Protocol
- [ ] Local LLM inference with streaming responses
- [ ] Semantic search across spaces (HNSW + BM25)
- [ ] Context-aware preference resolution
- [ ] Flow (smart clipboard) with Translation engine
- [ ] AI-triaged attention management with notifications
- [ ] Conversation bar for natural language interaction
- [ ] Identity system with PQC keys and credential isolation
- [ ] **Gate 4**: Intelligence stack complete

### Tier 4 Complete (Week 34)

- [ ] Intent verification with DIFC taint tracking
...
- [ ] **Gate 5**: Hardware-ready

### Tier 6 Complete (Week 56)
