...
**Related:** [layers.md](./layers.md) — Eight defense layers, [capabilities.md](./capabilities.md) — Capability system internals, [hardening.md](./hardening.md) — Crypto and ARM hardware
...
|---|---|---|
| Layer 1: Intent Verification | Medium | Priority fence isolates security path from resource path ([§2.1](./layers.md)) |
| Layer 2: Capability Check | None | Every AIRS directive still passes kernel capability validation |
| Layer 3: Behavioral Monitoring | Medium | Kernel monitors AIRS directive behavior ([§2.3.1](./layers.md)) |
| Layer 4: Security Zones | None | Zone boundaries are structural — directives cannot cross zones |
| Layer 5: Adversarial Defense | Medium | Agent hints screened as untrusted input ([§2.5.1](./layers.md)) |
| Layer 6: Cryptographic Enforcement | None | AIRS never touches encryption keys or crypto operations |
| Layer 7: Provenance Recording | Low | Resource directives added to provenance chain ([§2.7.1](./layers.md)) |
...
The system is slower in this mode but **equally secure**. Security never depends on AIRS making correct resource decisions. This is enforced by the kernel fallback mechanism ([§2.3.1](./layers.md)): the kernel can unilaterally disable AIRS resource orchestration while keeping all security layers active.
...
|---|---|---|
| Pathological prefetching (prefetch everything, thrash memory) | **Performance degradation** | Kernel fallback mode disables prefetching ([§2.3.1](./layers.md)). No data breach. |
...
- **capabilities.md [§3.2](./capabilities.md)**: `revoke()` calls `kernel.invalidate_channels_for_capability(token_id)`
...
- **capabilities.md [§3.1](./capabilities.md)**: Token creation example uses mandatory expiry (`expires: now() + Duration::days(90)`) with trust-level TTL comment
- **capabilities.md [§3.1](./capabilities.md)**: `capability_create()` documented as rejecting missing expiry
...
Phase 11:  Mandatory capability expiry per trust level (Gap 2)
Phase 11:  Behavioral gating integration: AIRS → kernel rate limiting (Gap 3)
Phase 18:  Formal verification that revocation fully propagates
Phase 18:  Formal verification that resource limits bound kernel heap
```
...
- **coq-of-rust**: Translate Rust to Coq for theorem proving. Target: capability attenuation monotonicity proof ([§3.3](./capabilities.md)).
...
AIOS on CHERI:
- Software capability tokens ([§3](./capabilities.md)) gain hardware enforcement — forgery becomes architecturally impossible
...
- **Transient execution fuzzing**: Integrate speculative execution testing into the security test harness ([§8](./hardening.md))
...
|---|---|---|---|
| Post-quantum crypto | Phase 24+ | ML-DSA library (no_std) | Token signature size, peer protocol |
| ARM CCA integration | Phase 30+ | ARMv9.2+ hardware | Model/agent hardware isolation |
| Formal verification (Verus/Kani) | Phase 17+ | Toolchain maturity | Provable security properties |
| CHERI hardware caps | Phase 36+ | CHERI AArch64 silicon | Hardware-enforced capabilities |
| AI-driven security (AIRS) | Phase 14+ | AIRS runtime | Adaptive threat detection |
| AI-driven security (kernel) | Phase 10+ | Offline training pipeline | Syscall anomaly detection |
| Spectre-BHB mitigations | Phase 7+ | ARMv9.4+ | Side channel defense |
| Static verification (LionsOS) | Phase 22+ | Build tooling | Compile-time security proofs |
