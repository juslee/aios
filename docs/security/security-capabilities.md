...
**Related:** [security-layers.md](./security-layers.md) — Defense layers (Layer 2 is capability check), [security-hardening.md](./security-hardening.md) — Formal verification targets, [security-operations.md](./security-operations.md) — Zero trust gap analysis
...
//    (see [§10.3](./security-operations.md) Gap 2). capability_create() rejects expires: None.
//
...
impl CapabilityTable {
    ...
    /// all access, including cached channel access (see [§10.3](./security-operations.md) Gap 1).
...
4. **Full audit trail.** The `resolution_log` records exactly which profile contributed which capability, enabling post-incident forensic analysis via the provenance chain (Layer 7, [§2.7](./security-layers.md)).
