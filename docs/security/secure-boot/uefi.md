...
ARM TrustZone provides a hardware-isolated secure world for key storage, cryptographic operations, and anti-rollback counters. Phase 24 implements an SMC-based interface to ARM Trusted Firmware (BL31); Phase 27+ provides a migration path to OP-TEE Trusted Applications for complex operations.

### §5.1 Key Migration

Before Phase 24, all cryptographic keys reside in kernel memory (pinned, no-dump pages). This protects against software-only attackers but not physical access (cold boot, DMA attacks). Phase 24 migrates critical keys to the TrustZone secure world.

**Keys migrated to secure world:**

| Key | Pre-Phase 24 Location | Phase 24 Location | Purpose |
|---|---|---|---|
...
**Migration process (Phase 24 M74):**

1. At first boot after Phase 24 update, kernel detects TrustZone availability via SMCCC (SMC Calling Convention) probing
...
## §5.7 OP-TEE Migration Path (Phase 27+)

Phase 24 implements a minimal SMC-based interface to ARM Trusted Firmware (BL31). For Phase 27 (hardware certification) and beyond, a migration path to OP-TEE Trusted Applications (TAs) enables more complex secure-world operations.

**Why migrate to OP-TEE:**

| Capability | BL31 SMC (Phase 24) | OP-TEE TA (Phase 27+) |
|---|---|---|
...
1. **Phase 24:** SMC interface to BL31 (seal, unseal, counter, sign, verify, encrypt, decrypt)
2. **Phase 27:** Deploy OP-TEE alongside BL31; wrap existing SMC services as OP-TEE TAs
3. **Phase 27+:** New features implemented as OP-TEE TAs:
...
pub enum SecureBackend {
    /// Direct SMC to BL31 (Phase 24)
    Bl31Smc(TrustZoneService),
    /// OP-TEE session (Phase 27+)
    OpTee(OpTeeSession),
