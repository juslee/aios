...
4. **Format.** This ADR holds the decisions, the design and the plan. The detail moves to the companion plan.
...
- **Reapable is not off-CPU.** Reaping frees nothing today. Before `UserAddressSpace` gets a `Drop` or ASID release, reaping must wait for a per-process on-CPU count (crash-fix step 6a).
...
- **Notification timeouts.** `check_notification_timeouts` collects the expired tids under `NOTIFY_DEADLINES.try_lock`, resets them, and calls `unblock` only after releasing the lock. It no longer tests thread state or cleans up waiter records, so no expiry is dropped (crash-fix F5's third bullet, `:180`), and `NOTIFY_DEADLINES` becomes the leaf that `deadlock-prevention.md` §3.3 already lists.
...
- `channel_create_unchecked` moves from `ipc/tests/mod.rs:302-322` into `ipc/mod.rs`, shares the allocator, and sets `creation_cap` and `creator` to `None`. It stays for init code before `enter_scheduler` that names synthetic creator tids (`service/mod.rs:209`, `storage/space.rs:144`, `gpu/service.rs:592`, `compositor/service.rs:592`, `ipc/tests/mod.rs:142`, `:217`) and for the select_cap test's inaccessible channel. Their ambient `ChannelAccess` grants stay: the kernel acts as the Service Manager until `CapabilityTransfer` exists. Kernel channels never cascade.
...
- **Rule** (doc comment on `grant_to_process`): at runtime, an object-bound token comes only from `create_with_access` or `shared_memory_share`. Ambient grants of them are allowed only in init code before `enter_scheduler` and in self-tests.
...
- The wake token adds no lock, edge, `DAIFClr` site or `THREAD_TABLE` hold with IRQs on. One such hold goes (`select.rs:296`), and the converted waits' IRQs-on `CURRENT_THREAD` read (`current_thread_id()`) becomes the masked `current_tid()`.
...
| `kernel/src/compositor/service.rs:547`, `kernel/src/ipc/channel.rs:30` | Lock-order comment; `timeout_ticks` doc comment |
...
- Killed borrowers' frames drain for the rest of the boot. Releasing them needs crash-fix 6a's per-process on-CPU count and per-mapping borrow counts, because borrows are counted per region. Reaping while threads are on a CPU, and address-space teardown.
...
- **8:** adds the exit safe point (§7), whose `prepare_block` check never leaves the CPU, and, once it exists, counts Dead threads in `bw`. `WAIT_STATE` may move into `ThreadInfo`, and `prepare_block` and `cancel_block` then read the current tid there. The sleeping-while-atomic check at `block_current` entry runs before `block_action`, so a pending token cannot hide it. Region borrowers dereference with no lock held, so the bench loop stays preemptible.
...
- **Keeping `SELECT_WAITERS` and deciding wakes by registration:** rejected; the sources are authoritative. **Clearing `WAKEUP_ERRORS` at wait entry** (#186): not adopted; results come from state, and the thread-side `WAKEUP_ERRORS` holds (crash-fix H3) do not grow (§9).
