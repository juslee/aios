...
pub enum DragFlowRequest {
    ...
    /// to enforce focus-gated clipboard access (§15.7 in extensions.md).
    DragStarted {
...
| Pull with target: Any | FlowRead + compositor focus | Background agents without focus are blocked; see §15.7 in [extensions.md](./extensions.md) |
| Handle action content | FlowActionHandle + user confirmation | Required when receiving `SemanticType::Action`; see §15.8 in [extensions.md](./extensions.md) |
