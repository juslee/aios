...
pub struct Version {
    ...
    /// Second parent for merge commits (Phase 13c — Space Sync conflict resolution).
    ...
    /// NOTE: Added to the struct now (as None) to avoid schema migration in Phase 13c.
...
Branch creation is implicit — it happens whenever Space Sync (§8) discovers that both the local and remote DAGs have advanced past a common ancestor. The `SyncConflict` struct (§8) represents a detected fork, and resolution produces a merge version node with two parents. The `Version` struct (§5.1) already includes a `merge_parent: Option<Hash>` field (added early to avoid schema migration in Phase 13c) — a second parent, only set for merge commits. Single-device operation always produces a linear chain (every version has at most one parent; `merge_parent` is `None`).

Conflict resolution strategies are defined in §8. Branching semantics are Phase 13c work — single-device operation (Phase 4a-4l) always produces a linear chain.
