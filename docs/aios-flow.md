...
**Related:** [aios-compositor.md](./aios-compositor.md) — Drag/drop integration, [aios-subsystem-framework.md](./aios-subsystem-framework.md) — DataChannel/Flow pipes, [aios-agents.md](./aios-agents.md) — SDK FlowClient, [aios-spaces.md](./aios-spaces.md) — History storage, [aios-experience.md](./aios-experience.md) — Flow Tray UI
...
The Flow Tray (see [aios-experience.md](./aios-experience.md)) provides the user-facing history interface:
...
Every subsystem's DataChannel (see [aios-subsystem-framework.md](./aios-subsystem-framework.md)) has a `connect_flow()` method. This is the bridge between hardware data streams and the Flow system:
...
**Transport:** Cross-device Flow uses the AIOS Peer Protocol (defined in [aios-networking.md](./aios-networking.md)). Content is encrypted in transit using the shared identity's keys. The Peer Protocol handles device discovery, connection establishment, and reliable delivery.
